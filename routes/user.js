...
router.get('/profile', verifyToken, (req, res) => {
    ...
    db.query(query, [userId], (err, result) => {
        ...
        if (result.length > 0) {
            // Sanitize the fullName before sending it in the response
            const sanitizedFullName = sanitizeInput(result[0].fullName);
            return res.json({ fullName: sanitizedFullName });
        } else {
